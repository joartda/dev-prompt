...
Agent는 단순히 코드를 생성하는 것을 넘어, **계획-실행-검증-수정(Plan-Execute-Reflect)** 루프를 통해 자율적으로 문제를 해결합니다.

- **계획(Plan)**: 작업을 시작하기 전에 생각의 사슬(Chain of Thought)을 출력하여 접근 방식을 투명하게 공유합니다.
- **실행(Execute)**: 계획에 따라 코드를 작성하거나 수정합니다.
- **검증(Reflect)**: 테스트 실행, 코드 리뷰, 보안 검사 등으로 결과물을 스스로 검증합니다.
- **수정(Iterate)**: 실패나 오류가 발견되면 원인을 분석하고 자동으로 수정을 시도합니다. 3회 이상 자체 수정 실패 시 사용자에게 도움을 요청합니다.

### 컨텍스트 및 프롬프트 모듈화
프로젝트 규모가 커질수록 AI 에이전트의 지침이 방대해져 컨텍스트 윈도우 초과나 주의력 분산 문제가 발생할 수 있습니다. 이를 방지하기 위해:

- 기술 스택, 도메인, 역할별로 지침을 모듈화하여 관리합니다 (예: `.cursorrules`, `frontend.prompt`, `backend.prompt`).
- 작업 성격에 따라 필요한 지침만 동적으로 컨텍스트에 주입합니다.
- 대화가 길어질 경우 핵심 컨텍스트를 자체 요약하여 유지합니다.

### 웹 검색 및 정보 수집

#### 검색 시점
아래 중 하나에 해당할 때 검색합니다.
...
- 그 외 LLM 스스로 필요하다고 판단되는 경우 적극적으로 검색을 통해 정보를 찾습니다.
...
| 4 | 기타 | 블로그, 개인 문서 (출처 반드시 명시), 검색 엔진 |
...
> **왜 묻는가**: 프로젝트가 앞으로 얼마나 확장될지에 따라 초기 아키텍처 투자 수준이 달라집니다. 확장 규모가 클수록 모듈화, 인터페이스 분리, 확장 지점 설계에 더 많이 투자해야 장기적인 재작성 비용을 피할 수 있습니다.

| 선택지 | 설명 | 예시 | 추천 접근 |
|--------|------|------|-----------|
| A. 기능 소수 (단일 목적, 완결형) | 한정된 기능, 더 이상 확장되지 않음 | 행사 등록 페이지, 사내 도구, 1회성 마이그레이션 스크립트 | 레이어드, 테스트 70%, 문서 최소화 |
| B. 지속 확장 (MVP → 고도화) | 초기 출시 후 기능이 계속 추가될 예정 | 스타트업 초기 제품, 사내 시스템 고도화, 커머스 MVP | 모듈식 모놀리스, 핵심 인터페이스 정의, 테스트 80%, ADR 기록 |
| C. 대규모 확장 (플랫폼화) | 여러 팀이 장기간 기능을 추가하며 독립적인 서비스로 확장 | 대형 SaaS, 마이크로서비스 생태계, 오픈소스 프레임워크, 글로벌 플랫폼 | 헥사고날 또는 MSA 준비, 테스트 90%+, 전체 문서, ADR 필수 |
...
| I. 벡터 DB | 벡터 검색 | pgvector, Pinecone, Weaviate, Milvus — AI/ML 임베딩 검색, 의미 검색 |
| J. 복합 사용 | (예: PostgreSQL + Redis) | |
...
| F. 엣지 컴퓨팅 (Cloudflare Workers, Fastly Compute@Edge) | 전 세계 초저지연 실행 | 글로벌 서비스, CDN 수준 응답 속도 필요 |
...
4. **Service Mesh**: Istio, Linkerd를 통한 서비스 간 mTLS 및 트래픽 제어
5. **시크릿 관리**: HashiCorp Vault, AWS Secrets Manager 등
6. **GitOps (K8s 배포)**: ArgoCD, Flux를 통한 선언적 배포
7. **내부 개발자 플랫폼(IDP)**: Backstage 등 개발자 포털 구축
8. **선택 안 함**: 수동 배포 또는 PaaS의 기본 기능 사용
...
3. **소프트웨어 공급망 보안**: SBOM 생성(Syft, bom), 컨테이너 이미지 서명(Cosign), SLSA 프레임워크 적용
4. **카오스 엔지니어링**: Chaos Mesh, LitmusChaos를 통한 시스템 회복 탄력성 검증
5. **선택 안 함**: 테스트 자동화 > 부하 및 성능 테스트의 기본 단위/통합 테스트만 진행
...
> - **클라우드 기반 개발 환경**: GitHub Codespaces, Gitpod — 로컬 설정 없이 즉시 개발 가능, 팀 환경 일관성 확보
...
| gRPC API | Go / Rust | PostgreSQL | — | tonic, prost, grpcio |
...
| AI / ML 서비스 | Python | PostgreSQL + pgvector | FastAPI + Celery | LangChain, HuggingFace, Ray, MLflow, BentoML |
| AI/ML 모델 서빙 | Python | — | FastAPI, BentoML, Ray Serve, Triton | MLflow, DVC, Weights & Biases |
...
테스트:     pytest · pytest-asyncio · factory-boy · Faker · testcontainers-python
정적 분석:  ruff · mypy · bandit
MLOps:      MLflow · DVC · Weights & Biases · BentoML · Ray Serve · Feast · Great Expectations
```
...
테스트:     Jest · Vitest · Supertest · MSW · testcontainers-node
...
테스트:     testify · mockery · go-sqlmock · testcontainers-go
...
테스트:     내장 #[test] · rstest · mockall · testcontainers-rust
...
### 마이크로서비스 및 분산 시스템 특화 패턴

| 패턴 | 설명 | 적용 예 |
|------|------|---------|
| Saga | 분산 트랜잭션의 일관성 보장 (보상 트랜잭션) | 주문-결제-재고 시스템 |
| Outbox | 이벤트 발행과 DB 변경의 원자적 처리 | CDC(Change Data Capture) 연계 |
| BFF (Backend for Frontend) | 클라이언트별 최적화된 API 게이트웨이 | 모바일/웹별 전용 백엔드 |
| API Gateway | 단일 진입점, 인증, 라우팅, 속도 제한 | Kong, KrakenD, Apache APISIX |
| Service Mesh | 서비스 간 통신의 관측·보안·제어 | Istio, Linkerd |
| Strangler Fig | 레거시 시스템의 점진적 마이그레이션 | 모놀리스 → MSA 전환 |
...
| **소프트웨어 공급망 보안** | Syft, bom (SBOM 생성), Cosign (이미지 서명), SLSA 프레임워크 | 빌드 파이프라인 무결성, 의존성 투명성 확보 |
| **내부 개발자 플랫폼(IDP)** | Backstage | 대규모 팀의 서비스 카탈로그, 문서, 템플릿 중앙화 |
...
커밋된 git log 에서도 해당 파일의 변경 내용은 절대로 읽지 않습니다.
...
| ML 모델 버전 불일치 | 모든 학습 실행에 `MLflow` 또는 `DVC`로 버전 태깅, 모델 레지스트리 운영 |
| 학습-서빙 불일치 (Training-Serving Skew) | 특징 저장소(Feature Store)로 변환 로직 통일, 온라인/오프라인 특징 일관성 검증 |
| 모델 성능 저하 (Data Drift) | 프로덕션 모델의 입력 분포 모니터링, Great Expectations으로 데이터 품질 검증 자동화 |
...
| 기법 | 설명 | 적용 시점 |
|------|------|-----------|
| **속성 기반 테스트 (Property-based Testing)** | 무작위 입력을 생성하여 함수의 불변성 검증 | 계산·변환·직렬화 로직 |
| **스냅샷 테스트 (Snapshot Testing)** | 출력 결과를 직렬화하여 예상치 못한 변경 감지 | UI 컴포넌트, API 응답, 설정 파일 |
| **시각적 회귀 테스트 (Visual Regression Testing)** | UI 스크린샷을 픽셀 단위 비교 | 프론트엔드 컴포넌트, 디자인 시스템 |
| **변이 테스트 (Mutation Testing)** | 코드에 작은 변형을 주입하여 테스트 품질 측정 | 핵심 비즈니스 로직, 정기적 품질 감사 |
| **카오스 엔지니어링 (Chaos Engineering)** | 프로덕션 환경에 장애 주입하여 회복 탄력성 검증 | 대규모 분산 시스템, 고가용성 요구 |
...
| 언어 | 단위/통합 | 비동기 | 목(Mock) | 컨테이너 | 속성 기반 |
|------|-----------|--------|----------|----------|-----------|
| Python | pytest | pytest-asyncio | unittest.mock, pytest-mock | testcontainers-python | Hypothesis |
| TypeScript | Jest / Vitest | 내장 | Jest Mock, MSW | testcontainers-node | fast-check |
| Go | testing (내장) | 내장 + `-race` | testify/mock, mockery | testcontainers-go | rapid (내장) |
| Rust | 내장 `#[test]` | `#[tokio::test]` | mockall | testcontainers-rust | proptest |
| Java/Kotlin | JUnit 5 | Awaitility | Mockito | Testcontainers | jqwik |
| C# | xUnit / NUnit | 내장 async | NSubstitute | Testcontainers .NET | FsCheck |
| PHP | PHPUnit / Pest | — | Mockery | — | Eris |
| Elixir | ExUnit (내장) | 내장 | Mox | — | StreamData |
...
아키텍처 다이어그램은 이미지가 아닌 코드로 작성하여 Git으로 변경 이력을 추적할 수 있도록 합니다.

| 도구 | 특징 | 적용 대상 |
|------|------|-----------|
| **Mermaid** | 마크다운 내장, GitHub 네이티브 지원 | 간단한 시퀀스·플로우·ERD |
| **PlantUML** | 풍부한 UML 다이어그램 지원 | 클래스·컴포넌트·배포 다이어그램 |
| **Structurizr DSL** | C4 모델 전용 DSL | 시스템 컨텍스트·컨테이너·컴포넌트 다이어그램 |
| **D2** | 현대적 선언형 다이어그램 언어 | 인프라·네트워크 토폴로지 |

### ADR (Architecture Decision Records) 작성

중요한 아키텍처 결정은 `adr-tools` 또는 유사한 도구를 사용하여 일관된 형식으로 기록합니다. 각 ADR은 결정의 맥락, 고려한 대안, 결정 내용, 결과 및 트레이드오프를 포함합니다.

### AI 친화적 문서화 (LLM-Friendly Formatting)

AI 에이전트가 문서의 규칙과 제약 조건을 정확히 파싱할 수 있도록, 핵심 지침은 구조화된 태그로 구분합니다.

- 필수 보안 규칙은 `<rules type="security" severity="critical">...</rules>` 내에 배치
- 코딩 컨벤션은 `<constraints>...</constraints>` 태그로 감싸 명확히 구분
- 금지된 패턴은 `<avoid>...</avoid>` 태그로 명시적 표시
- 권장 사항은 `<recommend>...</recommend>` 태그로 구분

### 자동화된 지식 관리

- 코드베이스에서 자동으로 최신 API 문서를 생성하는 파이프라인 구성
- 에러 코드 사전과 상태 코드 매핑을 코드로부터 자동 추출
- 생성형 AI를 활용하여 릴리스 노트와 변경 로그 초안 자동 생성
...
| 소프트웨어 공급망 | SBOM 생성(Syft, bom), 컨테이너 이미지 서명(Cosign), SLSA 프레임워크 적용 |
| Zero Trust | 네트워크 경계 기반 신뢰 제거, 모든 요청을 인증·인가 |
...
- 컨테이너 이미지 스캔: Trivy, Snyk를 CI 파이프라인에 통합
...
별도의 사용자의 요구사항이 없다면 기본적으로 아래와 같이 처리한다.
...
```protobuf
// 표준 gRPC 상태 코드와 google.rpc.Status 사용
// code: NOT_FOUND (5), INVALID_ARGUMENT (3), INTERNAL (13) 등
// details에 ErrorInfo 포함
```
...
1. SAST 도구 실행
2. 의존성 취약점 검사
3. 컨테이너 이미지 스캔 (Trivy/Snyk)
4. 보안 규칙 > 공통 보안 체크리스트 대조
5. 발견된 취약점 목록 + 심각도 + 수정 방안 보고

#### `generate sbom`
1. 언어·패키지 관리자에 맞는 SBOM 생성 도구 선택 (Syft, bom, cyclonedx 등)
2. SBOM 생성 및 `.ai.dev/` 폴더에 저장
3. 생성된 SBOM 요약 보고 (패키지 수, 라이선스, 취약점 존재 여부)
...
1. 현재 에러 처리 방식 분석
2. RFC 7807 형식으로 변환 계획 수립 (REST) 또는 GraphQL/gRPC 형식
3. 외부 노출 민감 정보 제거
...
4. SBOM 생성 및 컨테이너 이미지 서명 단계 포함
5. `.github/workflows` 등에 파일 생성 및 저장
...
1. ML 모델 학습 파이프라인 식별
2. MLflow/DVC를 통한 실험 추적 및 모델 버전 관리 설정
3. 특징 저장소(Feast 등) 연동 검토
4. 모델 서빙 인프라(BentoML, Ray Serve, Triton) 구성 제안
5. 데이터 품질 검증(Great Expectations) 및 모니터링 파이프라인 구성
...
```
0.  .ai.dev/ 폴더 및 DECISIONS.md 생성           → 프로젝트 최초 실행 시 가장 먼저 수행
0-1. AGENT_CONFIG.md 확인·생성                   → 모델 자가 식별, 없으면 사용자 1회 확인
0-2. CODEBASE_PROFILE.md 확인·생성               → 코드베이스 전체 스캔 및 프로파일 캐시
1.  초기 질문 및 프로젝트 정의                       → Q0에서 유형 확인 후 Q1~Q16 그룹별 진행 및 DECISIONS.md 갱신
2.  개발 환경 및 IDE 추천                           → 개발 환경 설정
3.  방법론·아키텍처 결정                             → .ai.dev/ARCHITECTURE.md 초안
4.  기술 스택 및 라이브러리 선정                      → README.md 작성 (사용자 협력)
5.  보안 규칙 문서화                                 → .ai.dev/SECURITY.md
6.  프레임워크 가이드라인                             → .ai.dev/FRAMEWORK_GUIDELINES.md
7.  핵심 모듈 구현 (TDD)                            → 테스트 → 코드 → 리팩토링 (마이그레이션 시 기존 분석 선행)
8.  에러 처리 표준화                                 → .ai.dev/ERRORS.md
9.  로깅 추가
10. 문서 동기화                                      → .ai.dev/ 내 관련 파일 갱신
11. Git 커밋                                        → Conventional Commits + PR
12. 빌드·메모리 최적화
13. 보안 감사                                        → SAST + 의존성 검사 + SBOM 생성
14. 최종 문서 일관성 검사                             → .ai.dev/ 내 모든 문서와 코드 정합성 확인
15. 인프라/CI-CD 파이프라인 구성 및 부하 테스트 스크립트 작성 (사용자 선택 시)
16. MLOps 파이프라인 구성 (AI/ML 프로젝트 시)
17. 컨텍스트 압축 및 아카이브 (압축 조건 충족 시)      → .ai.dev/context/ 에 세션 보존
```
...
작업 완료 시마다 아래 절차를 자동으로 수행하세요
1. 현재 변경사항과 관련된 문서·코드·구조의 상태를 분석한다.
2. 발견된 불일치, 누락, 개선 기회, 영향받는 다른 요소를 구체적으로 서술한다.
3. 그에 따른 구체적인 변경을 추천하고, 왜 그 변경이 필요한지 이유를 명확히 설명한다.
4. 추천한 변경을 진행할지 사용자에게 확인한다. (진행 / 수정 / 건너뛰기)

분석 시 다음 관점을 자동으로 점검하세요:
- 결정·구조의 일관성 및 최신성
- 영향받는 다른 문서·코드 존재 여부
- 용어·스타일·컨벤션 일치 여부
