...
```
### 1-1. Tool 역량 진단
Agent가 정상 동작하려면 다음 Tool이 필수로 필요합니다. 각 Tool 사용 가능 여부를 스스로 진단하세요. Tool 미지원 시 즉시 경고를 출력합니다.

|Tool|필수|플랫폼|설명|
|-----|:---:|:-------:|------|
|파일 읽기/쓰기 (Read, Write, Edit)|필수|전체|코드 생성 및 코드베이스 분석|
|쉘 명령어 (Bash, Shell, Terminal, cmd)|필수|전체 (Windows: WSL/Git Bash/PowerShell)|빌드, 테스트, Git, 패키지 설치|
|코드 검색 (Grep, Search, Glob)|필수|전체|코드베이스 탐색 및 리팩토링|
|도구 호출 (Function Calling)|필수|전체|Agent 기반 Tool 사용|
|웹 접근 (Web Search, URL Fetch)|권장|전체|라이브러리 검증, 버그 원인 분석|

쉘 명령어는 가능한 PowerShell Core(`pwsh`) 등 플랫폼 간 일관된 인코딩을 지원하는 툴을 우선 사용합니다. Windows에서 `cmd.exe`를 사용해야 한다면 `chcp 65001`로 UTF-8 전환 후 실행하고, 출력이 깨질 경우 코드 페이지를 감지하여 재시도합니다.

진단 결과 모든 필수 Tool 사용 가능 시: 정상 진행합니다.
하나라도 미지원 시 다음을 출력합니다:
```
[경고] 현재 환경에서 다음 Tool을 지원하지 않습니다: {누락 Tool 목록}
- 사용 가능: {가능 Tool 목록}
- 미지원: {누락 Tool 목록}
필수 Tool 누락으로 Agent 기능이 제한됩니다. 지원 환경으로 전환하거나
해당 기능이 필요 없는 작업으로 범위를 조정해 주세요.
```
진단 결과는 `.ai.dev/AGENT_CONFIG.md`에 함께 기록합니다.
...
- calibration_count: 0
- tools_available: [read, write, shell, search, function_calling]
- tools_missing: []
- fully_capable: true
```
## 2. 프로젝트 상태 확인
