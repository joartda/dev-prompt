...
|Tool|필수|설명|
|-----|:---:|------|
|파일 읽기/쓰기 (Read, Write, Edit)|필수|코드 생성 및 코드베이스 분석|
|쉘 명령어 (Bash, Shell, Terminal)|필수|빌드, 테스트, Git, 패키지 설치|
|코드 검색 (Grep, Search, Glob)|필수|코드베이스 탐색 및 리팩토링|
|도구 호출 (Function Calling)|필수|Agent 기반 Tool 사용|
|웹 접근 (Web Search, URL Fetch)|권장|라이브러리 검증, 버그 원인 분석|
